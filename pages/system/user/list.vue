	...
	<view class="fix-top-window">
		<view class="uni-header">
			...
			<view class="uni-group">
				...
				<button v-if="hasPermission()" class="uni-button" type="primary" size="mini" @click="navigateTo('./add')">{{ $t('common.button.add') }}</button>
				<button v-if="hasPermission()" class="uni-button" type="warn" size="mini" :disabled="!selectedIndexs.length" @click="delTable">
		...
		<view class="uni-container">
			...
			>
				<uni-table ref="table" :loading="loading" :emptyText="error.message || $t('common.empty')" border stripe type="selection" @selection-change="selectionChange">
					<uni-tr>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'username')" sortable @sort-change="sortChange($event, 'username')">
							用户名
						</uni-th>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'nickname')" sortable @sort-change="sortChange($event, 'nickname')">
							用户昵称
						</uni-th>
						<uni-th align="center" filter-type="select" :filter-data="options.filterData.status_localdata" @filter-change="filterChange($event, 'status')">
							用户状态
						</uni-th>
						...
						<uni-th
							...
							@filter-change="filterChange($event, 'last_login_date')"
							sortable
							@sort-change="sortChange($event, 'last_login_date')"
						>
							最后登录时间
						</uni-th>
					...
					<uni-tr v-for="(item, index) in data" :key="index">
						...
						<uni-td align="center"><uni-dateformat :threshold="[0, 0]" :date="item.last_login_date"></uni-dateformat></uni-td>
						<uni-td align="center">
							<view class="uni-group">
		...
		<uni-popup ref="tagsPopup" type="center">
			<view class="tags-manager--x">
				...
				></uni-data-checkbox>
				<view class="uni-group"><button @click="managerMultiTag" class="uni-button" type="primary" style="margin-right: 75px;">保存</button></view>
			</view>
		</uni-popup>
		<!-- #ifdef H5 -->
		{{ smsReceiver }}
